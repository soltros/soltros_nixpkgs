...
def scan_all_packages(token: Optional[str] = None) -> List[Dict[str, Any]]:
    headers = get_headers(token)
    checker = PackageChecker(headers)
    results = []

    # 1. Addwater
    addwater_nix = REPO_ROOT / "pkgs" / "addwater" / "default.nix"
    ...
    # 2. ChatGPT
    chatgpt_nix = REPO_ROOT / "pkgs" / "chatgpt.nix"
    ...
    # 3. Flakebuilder
    flakebuilder_nix = REPO_ROOT / "pkgs" / "flakebuilder.nix"
    ...
    # 4. Flakebuilder-GUI
    flakebuilder_gui_nix = REPO_ROOT / "pkgs" / "flakebuilder-gui.nix"
    ...
    # 5. Hideout
    hideout_nix = REPO_ROOT / "pkgs" / "hideout" / "default.nix"
    ...
    # 6. Nixboutique
    nixboutique_nix = REPO_ROOT / "pkgs" / "nixboutique.nix"
    ...
    qst_nix = REPO_ROOT / "pkgs" / "quick-settings-tray" / "default.nix"
    if qst_nix.is_file():
    ...
    # 8. Waterfox
    waterfox_nix = REPO_ROOT / "pkgs" / "waterfox.nix"
    ...
    for local_pkg in ["pantheon-studio", "termsmith", "vpn-manager"]:
        pkg_dir = REPO_ROOT / "pkgs" / local_pkg
        if pkg_dir.is_dir():
            results.append({
                "package": local_pkg,
                "type": "Local Package",
...
def main():
    ...
    results = scan_all_packages(args.token)
    updates = [r for r in results if r.get("status") == "update_available"]
